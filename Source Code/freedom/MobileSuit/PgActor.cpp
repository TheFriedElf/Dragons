...
float PgActor::ms_fGravity = -800.0f;				// ĳ���� �����ÿ��� ����Ǵ� �߷�
...
PgActor::~PgActor()
{
	// ���⿡�� ActorManager�� AMPool�� ������ �ǹǷ� �� ���� ActorManager�� �ߴ� ������� �� ������.
	Terminate();
...
bool PgActor::CreateCopyEx(PgActor* pkNewActor)
{
	...
	pkAM->Update(0.0f);
	NiTimeController::StartAnimations(pkNewActor->GetNIFRoot(), 0.0f); //PgWorld�� Attach�� �� �˾Ƽ� �ȴ�.
...
void PgActor::RefreshHPGaugeBar(int iBeforeHP, int iNewHP, PgActor* pkAttacker, bool const bSetAbil)
{
	...
	//BigArea�϶� AT_HP ���� ��Ŷ�� ���� ���� HP�ٰ� ������ �ʴ� ��찡 �߻�
	//�׷��� �Ʒ��� ���� ������ �߰�����, ��ġ�� ���� ��� ǥ������?
	//��ġ�� ��� RefreshHPGaugeBar�� ȣ���ϱ� ���� üũ�Ͽ� �Լ��� �������� �ʵ��� ��
	//�� �� ���� ����� �ִٸ� �� ����� ���������� ������
...
bool PgActor::SetHPBarValue(int const iBefore, int const iNew)
{
	...
	{
		//Pet�� �̰����� ������� �ʵ��� ���´�
		//Pet�� m_pHPGaugeBar�� ǥ���ϴ� ������ HP�� �ƴ� MP �̱� ������ MP�� ����Ǵ� ������ ���� �������ش�.
		return false;
	}break;
	default:
	{
...
bool PgActor::Walk(BYTE byDir, float fSpeed, float fFrameTime, bool bCorrectFinalPos)
{
	...
	//		
	//		// BirthDelay�� Averange Latency���� Ŀ�� �Ѵ�.
	...
	if (m_bWalkingToTarget)
	{
		...
		//NiPoint3 kDiff = m_kTargetLoc - GetTranslate();	//	������ �ʴ� ����? �� �����ϴ°�? leesg213
		//kDiff.z = 0;	
...
void PgActor::SetFreezed(bool const bTrue, bool const bSetAni, bool const bDoDmgAction)
{
	...
	{// ������ �޶���
		...
		if (bSetAni)
		{
			if (GetFreezed())
			{
				SetAnimSpeedInPeriod(0.0001f, 99999999);	//	�ִϸ��̼� ����
			}
			else
			{
...
void PgActor::SetNoWalkingTarget(bool bDoNextAction)
{
	...
	m_kTargetDir = NiPoint3::ZERO;
	m_fDiffLength = 0.0f;
	m_bCheckCliff = false;	//	leesg213 �߰���
	m_fWalkToTargetLocSkillRange = 0.0f;
...
void PgActor::SetWalkingTargetLoc(NiPoint3 const& kTargetLoc, bool const bForceToTarget, char const* pcNextAction, bool bCheckCliff, float fSkillRange)
{
	...
	// bForceToTarget : Target���� �����̴� ���� ���� ĵ���� �ȵǰ� �� ���ΰ�.
...
void PgActor::MoveActorAbsolute(NxVec3 const& kMoveAbs)
{
	...
	//// m_kMovingAbsolute�� Controller�� move�� �ʿ��� ������.
	//// move��.. �� �Լ��� ����ǰ� ���� ���� �����Ӷ� ��� �Ǳ� ������..
	//// �ѹ��� �ʰ� �����δ�. �׷��� ������. actor�� �̹������� ���� ���� ������ �༭.
	//// ��� �̵� �Ȱ� ���� ����ó�� �����ش�.
...
void PgActor::StartWeaponTrail()
{
	...
	{
		PartsAttachInfo::iterator itr = m_kPartsAttachInfo.find(EQUIP_LIMIT_WEAPON);	//	5���� ����
		if (itr != m_kPartsAttachInfo.end())
		{
			PgItemEx* pkEquipItem = itr->second;
...
bool PgActor::StartTrail(eTrailAttachType const eTrailType, std::string const kTexPath, int iTotalTime, int iBrightTime)
{
	...
	//PgTrailNodePtr spTrailNode = GetTrailNode( eTrailType ); // NULL�� �ƴϸ� ������ �ȵȰ��̹Ƿ�
	//if( NULL != spTrailNode )
	//{// ���� �ϰ�
	//	return false;
	//}
	NiAVObject* pkNode = GetActorManager()->GetNIFRoot();	// ��Ʈ�� ���� �����ؼ�
	if (NULL == pkNode)
	{
		return false;
	}
...
void	PgActor::StartGodTime(float fTotalGodTime)
{
	...
	//RemoveStatusEffectFromActor2�� ����� �͵��� MadeByItem�� ���ش�.
...
bool PgActor::CheckCollWithCamera(float& fCollScreenX, float& fCollScreenY)
{
	...
	if (m_fCheckCam_FirstDValue * d_value < 0)	//	�� ���� �ε����°�?
	{
		// ī�޶� �� �ȿ� �ִ°�?
...
void	PgActor::AbilChangedByEffect(BM::GUID const& kCasterGUID, WORD wAbilID, int iAbilValue, int iDelta)
{
	...
	if (AT_HP == wAbilID || AT_MP == wAbilID)
	{
		...
		{
			if (iCurrent <= iNew && 0 != iCurrentValue) //hp�� ȸ���Ǵ°��̶��
			{
...
void	PgActor::ShowChatBalloon_Clear()
{
	...
	{
		m_bInputNow = false;
		m_pTextBalloon->SetExistTime(100);	//100/1000�� �Ŀ� ����ȴ�.
	}
}
...
// LoadType 0 : MyActor �̸� �ٷ� �ε�, �ƴϸ� Thread��
// LoadType 1 : �ϴ� AttachItemConatiner�� ����Ͽ�, �ϳ��� ���̴� ������� ó�� ������ Item�� Thread�� Ÿ�� ��.
// LoadType 2 : ������ �ٷ� �ε�.
bool PgActor::AddEquipItem(int iItemNo, bool bSetToDefaultItem, PgItemEx::ItemLoadType eLoadType, bool bReal)
{
	//! �Ѱ��� �Լ��� PG_USE_WORKER_THREAD�� ����Ǹ� 3���� �ɰ��� ������ ��Ȳ�� ���� �� �θ��� �ȴ�.
					...
					if (pkUnit)
					{
						...
						if (pkInven)
						{
							...
							if (kEquipPos >= EQUIP_POS_HAIR_COLOR
								&& kEquipPos <= EQUIP_POS_FACE) // �̰͵��� Enchant�� ���� Inven���� ����.
							{
								return true;
							}
	...
	if (eLoadType == PgItemEx::LOAD_TYPE_USEQUEUE && m_iEquipCount > 0 && IsMyActor() == false)
	{
		...
		kInfo.pItem = NULL;		// NULL�� ����� Item Loading Thread�� �ְ� �ȴ�.
...
bool PgActor::EquipItem(PgItemEx* pkEquipItem, int iItemNo, bool bSetToDefaultItem)
{
	...
	{// ������ �������� 20�̻��̸�, Skinning�Ͽ� Attach�ϴ� �������̴�.
		bAttachSuccess = (AttachSkinningParts(pkEquipItem) > 0 ? true : false);
		if (GetNIFRoot())
		{// AttachSkinningParts()�ϸ鼭 �ʱ�ȭ�� HeadSize�� ����� ������ ������
			NiAVObject* pkHead = GetCharRoot()->GetObjectByName("Bip01 Head");
			if (pkHead)
			{
		...
		if (pkUnit)
		{
				...
				)
			{
				...
				if (pkSubplayerActor)
				{
					...
					//	{// ��� ������ Recv_PT_M_NFY_ITEM_CHANGE(BM::Stream* pkPacket)�� DISCT_SET_DEFAULT_ITEM���� ���� �ٲ����
	...
	pkEquipItem->SetActorNodesHide(this, true); //������ �������� ���� ��尡 ������ ������
	//////////////////////////////////////////////	
	// Ŀ�� Į��.
	...
	//! �������� �����Ͽ�����, UIModel�� ������Ʈ �ؾ� �ϹǷ� �÷��׸� TRUE�� ����
	m_bNeedToUpdateUIModel = true;

	// Material Prop�� �ٲ����Ƿ� �����ؾ� �Ѵ�.
	NeedToUpdateMaterialProp(true);
...
bool PgActor::GetCoupleItemColor(CONT_NI_COLOR& kContColor)
{
	...
	if (pkPlayer)
	{
		...
		{//GUID ��  ������, �������� XOR�� ����(������ GUID ������. ����, ������ �������)
			...
			size_t const max_color = sizeof(BM::GUID) / 3;//rgb �� 3
...
{
	if ((NiGetCurrentTimeInSec() - m_fLastCheckItemUseTime) < 5)	//	 5�ʿ� �ѹ����� üũ�Ѵ�.
	{
		return;
	}
...
bool PgActor::UnequipItem(EInvType kInvType, EEquipPos eEquipPos, int iItemNo, PgItemEx::ItemLoadType eLoadType, bool const bClear, bool bReal)
{
	...
	{// ���� ���� ������ ��ȣ�� �ְ�, ��¥ ���� ���� �ϴ°��̶��
		...
		if (0 < iSetNo)
		{// ��Ʈ �������� ���� ���� �Ǿ��°� Ȯ���ؼ�
			RemoveCompletedItemSet(iSetNo);		// ��Ʈ ������ �ϼ��� ����Ǵ� ȿ���� ���� ���ְ�
			RemoveItemEffect(iItemNo);			// �����ۿ� Ư���������� �ٿ��� ��ƼŬ ������ �־��ٸ� �������ְ�
		}
	}
	...
	if (bReal
		&& 0 == m_kItemEquipInfo.erase(iItemNo)	//(��� ������ �ܰ� �������� ������ �������)
		)
	{// ����� ���� ���� �Ǵ°����� Ȯ���Ͽ�  ���ŵȰ� ���ٸ� ���� �Ǿ��ִ� �������� �ƴϱ⿡ �������ְ�
		return false;
	}
	...
	{// ��ü���� �ܰ��� �ƹ��͵� ���ٸ�
		if (DetachParts(pkItemEx)					// ���� ���� ��ġ�� �ܰ��� �������ְ�
			&& EQUIP_LIMIT_HELMET == kEquipLimit	// �� �κ��� ����̶��
			)
		{
...
{// eInvType�� iEquipItemNo�� �����ɶ� ��Ʈ�������� �ϼ��Ǵ°��� �ִ°�
...
int PgActor::GetAppearanceItemNo(EEquipPos const eEquipPos, int& riType, bool const bCheckEffect, bool const bCheckCashItemSet, bool const bCheckNormalItemSet, bool const bCheckCash, bool const bCheckNormal)
{
	...
	// ������ �ϴ� �켱���� : Effect�ܰ� > Cash��Ʈ �߰� ������ �ܰ� > �Ϲݼ�Ʈ �߰� ������ �ܰ� > ĳ�þ����� �ܰ� > �Ϲݾ����� �ܰ� > ����Ʈ������ �ܰ�
	...
	bool const bShowAbleCashItem =
		(S_OK == pkInv->GetItem(IT_FIT_CASH, eEquipPos, kCashInvItem)	// ĳ�þ������� �����ϰ�
			&& !kCashInvItem.IsUseTimeOut()								// ������ ���ѽð��� ������ �ʾҰ�
			&& !kOption.IsHideCashInvenPos(eEquipPos)				// ���̱� �ɼǿ� ���߾������� �ʴٸ�
			);														// ĳ�þ������� ���̴� �������̴�.
	...
	bool const bShowAbleNormalItem =
		(S_OK == pkInv->GetItem(IT_FIT, eEquipPos, kNormalInvItem)			// �Ϲݾ������� �����ϰ�
			&& !kOption.IsHideEquipInvenPos(eEquipPos)				// ���̱� �ɼǿ� ���߾������� �ʴٸ�
			);															// �Ϲݾ������� ���̴� �������̴�
	...
	if (bCheckNormal
		&& (bShowAbleNormalItem && EQUIP_POS_MEDAL != eEquipPos)	// ��־����� �߿� �޴� ��ġ�� �ƹ��͵� ǥ�� ���� �ʱ⿡ �ܰ� �����۹�ȣ�� ��ȯ�ϸ� �ȵǰ�
		)
	{// �Ϲ� ������ �ܰ�
...
bool PgActor::AttachTo(int iSlot, char const* pcTargetName, NiAVObject* pkObject)
{
	...
	{// Pilot�� �����ϰ� (NPC��쿡�� ����) 
		...
		{// Unit�� �ִٸ�
			...
			if (IsRidingPet()						// �꿡 Ÿ�� �ִ�
				&& pkUnit->IsUnitType(UT_PLAYER)	// �÷��̾���
				)
			{// ���� ��ü�� ��
...
void PgActor::MoveParticlesToTarget(PgActor* pkTarget)
{
	...
	while (itAttachSlotSrc != m_kAttachSlot.end())
	{
		...
		pkTarget->m_kAttachSlot.insert(*itAttachSlotSrc); //������� ��ƼŬ �̵��ϰ� ���� ��Ͽ��� ����
	...
	while (itAttachSlotNoZTestSrc != m_kAttachSlot_NoZTest.end())
	{
		...
		pkTarget->m_kAttachSlot_NoZTest.insert(*itAttachSlotNoZTestSrc); //������� ��ƼŬ �̵��ϰ� ���� ��Ͽ��� ����
...
bool PgActor::DetachFrom(int iSlot, bool bDefaultThreadDelete)
{
	...
	if (itr != pkActor->m_kAttachSlot.end())	//	�Ϲ� ���Կ� ������
	{
	...
	itr = pkActor->m_kAttachSlot_NoZTest.find(iSlot);	//	Z���� ���Կ� �ִ��� ã��
...
void	PgActor::SetAlpha(float fAlpha)
{
	...
	if (m_spAlphaProperty)
	{
		...
		{
			SetDefaultMaterialNeedsUpdateFlag(false);	//	���̴��� ������Ʈ ���Ѿ��Ѵ�.
		}
...
bool PgActor::FindMaterialProp(NiNode* pkRoot, bool bCheckNoChange, bool bTraverseAllNode)
{
	...
	for (unsigned int i = 0; i < uiArrayCount; ++i)
	{
		...
		if (bCheckNoChange)
		{
			...
			if (pkExtra)
			{
				// Geometry�� NoColorChange��� UserProp�� ���������, �׿� �ش��ϴ� Material�� ����Ʈ�� �߰����� �ʴ´�.
...
NxVec3 PgActor::GetAdjustValidDetailDeltaLoc(NxVec3 kTargetDeltaLoc, NxVec3 kFindLoc, float fDegree)
{
		...
		(iCount < PG_GET_DELTALOC_COUNT))
	{
		...
		kDelta = kMat * kDelta;	//	�̵����͸� fDeltaDegree ��ŭ Z���� �߽����� ȸ����Ų��.

		NxExtendedVec3 kAfterPos = kCharPos;
		kAfterPos += kDelta;	//	ȸ���� �̵����͸�ŭ ������ ��ǥ�� ���Ѵ�.
		...
		{
			kDeltaLoc = kDelta;
			fDeltaDegree -= fHalfDegree;	//	fDeltaDegree �� ������ ���δ�.
		}
		else
		{
			fDeltaDegree += fHalfDegree;	//	fDeltaDegree �� ������Ų��.
		}
...
NxVec3 PgActor::GetAdjustValidDeltaLoc(NxVec3 kTargetDeltaLoc)
{
	...
	else if (pkScene)
	{
		...
		kDelta1 = kMat1 * kDelta1;	//	�̵����͸� Z���� �߽����� 50�� ȸ��
		NxVec3 kDelta2 = kTargetDeltaLoc;
		kDelta2 = kMat2 * kDelta2;	//	�̵����͸� Z���� �߽����� -50�� ȸ��

		NxExtendedVec3 kAfterPos1 = kCharPos;
		kAfterPos1 += kDelta1;	//	Z���� �߽����� 50�� ȸ����Ų �̵����͸�ŭ ����.
		NxExtendedVec3 kAfterPos2 = kCharPos;
		kAfterPos2 += kDelta2;	//	Z���� �߽����� -50�� ȸ����Ų �̵����͸�ŭ ����.

		if (	//	kAfterPos1 ��ǥ�� ĳ���Ͱ� �̵� �� �� �ִ°�?
			GetWorld()->raycastClosestShape(
				...
				NX_STATIC_SHAPES, kHit, -1, 100000.0f, NX_RAYCAST_SHAPE))
		{
			...
			kTargetDeltaLoc = GetAdjustValidDetailDeltaLoc(kTargetDeltaLoc, kDelta1, 50.0f);	//	�� ������ ��ǥ�� ã�´�.
#else
			kTargetDeltaLoc = kDelta1;
#endif
		}
		else if (	//	kAfterPos2 ��ǥ�� ĳ���Ͱ� �̵� �� �� �ִ°�?
			GetWorld()->raycastClosestShape(
		...
		{
#ifdef PG_USE_DETAIL_DELTALOC
			kTargetDeltaLoc = GetAdjustValidDetailDeltaLoc(kTargetDeltaLoc, kDelta2, -50.0f);	//	�� ������ ��ǥ�� ã�´�.
#else
...
void PgActor::UpdatePhysX(float fAccumTime, float fFrameTime)
{
	...
	Update_IsAbleSlide(fAccumTime, fFrameTime, bDoNotSlide);	// ���鿡�� �̲������Ⱑ �������� üũ �ϰ�

	if (m_fSpecifiedFrameTime != 0.0f)	//	�̰Ŵ� ���� ������ �ʴ´�.
	{
	...
	if (!GetFreeMove())
	{
		...
		if (IsMeetFloor())
		{
			...
			if (bDoNotSlide == false && m_kControllerShapeHit.shape)
			{
				...
				//NxCollisionGroup kGroup = m_kControllerShapeHit.shape->getGroup(); // �������� ������ ũ���� �κ�. ���� ������� �ʾƵ� �����ϱ⿡ ������
				...
				if ((91.0f >= fAngleDeg && PG_LIMITED_ANGLE < fAngleDeg)
					//&& (kGroup == PG_PHYSX_GROUP_BASE_FLOOR || kGroup == PG_PHYSX_GROUP_BASE_WALL || kGroup == PG_PHYSX_GROUP_OBJECT) // kGroup�� ���ü� �����Ƿ� ����
					)
				{// �� ���� Cross Product�� ���� ������ ���͸� ���Ѵ�.
					if (90.0f > fAngleDeg)
					{
	...
	if (GetJump())
	{
		// �ݵ�� getDebugPosition�� ��� �Ѵ�. move�� �� �� Update�� �Ǳ� ������ getPosition���� �������� ��ǥ�� �״�δ�.
	...
	if (bPreviousFloor != IsMeetFloor() && GetAction())	//	�ٴڿ��� �������ų� Ȥ�� �ٴڿ� ������� ��ũ��Ʈ ȣ���Ѵ�.
	{
...
bool PgActor::UnmountPet(void)
{
	...
	DetachNameNodes(m_pkMountedRidingPet->GetNIFRoot(), ATTACH_POINT_RIDENAME); //ž�¿� �̸���� �и�
	AttachNameNodes(pkNodeSceneRoot); //�̸� ��� ���� ����
	...
	m_pkMountedRidingPet->MoveParticlesToTarget(this); //�꿡�� ���� ��ƼŬ�� �����Է� �̵�..
	...
	lua_tinker::call<void, lwActor, bool>("SubActorHide", lwActor(this), true); //�ֵ��� ����
...
bool PgActor::MountPet(void)
{
	...
	if (pkActorPet == NULL || !IsCompleteLoadParts() || !pkActorPet->IsCompleteLoadParts()) //���ΰ� �� ��� �ε��� �Ϸ�Ǿ� �־�߸� �Ѵ�
	{
		return false;
	}
	//-------------Step2. ž�� ������ ������ üũ----------------------------
	...
	{ //���� ���ų� ���̵� ���� �ƴϸ�
		return false; //MSG: ž���� �� �ִ� ���� �����ϴ�.
	}

	//--Step3. ���� üũ (Ż �� �ִ� �����ΰ�?)
//	PgAction* pkAction = GetAction();
//	if(!pkAction/* || !IsMeetFloor() || pkAction->GetActionType() != "IDLE"*/) //���� ���鿡 ���ְų� ���̵� �����϶��� ����
//	{
//		return false; //MSG: ž���� �� ���� �����Դϴ�.
//	}



	//ž���� ���� �� ��ġ�� �̸���� ����
	if (m_pTextBalloon) //MountPet() �Լ��� �ι� �̻� ȣ��� ��츦 ����ؼ� üũ�ؾ� �Ѵ�.
	{
		m_pTextBalloon->SetEnable(false);
	}
	DetachNameNodes(GetNIFRoot()); //��� �и����� �̸���� �и�
	pkActorPet->DetachNameNodes(pkActorPet->GetNIFRoot());
	AttachNameNodes(pkActorPet->GetNIFRoot(), ATTACH_POINT_RIDENAME);

	//--Step4. ��Ʈ�ѷ� ����ȭ(���� ��Ʈ�ѷ��� PC�� ��Ʈ�ѷ��� ���� ������ ��ġ ��Ų��.)
	...
	NiActorManager* pkActorMng = GetActorManager();
	if (pkActorMng)
	{ //�� xml���� scale�� ������ Ű���ٸ� ž�� �� PC�� scale�� Ŀ���Ƿ� ž�� �� pc�� scale�� �̸� �ٿ��ش�.
		NiAVObject* pkNifRoot = pkActorMng->GetNIFRoot();
		if (pkNifRoot)
		{
	...
	ReserveTransitAction(ACTIONNAME_RP_IDLE); //PC�׼� ���̵��..
	m_pkMountedRidingPet->ReserveTransitAction(ACTIONNAME_RIDING); //��׼� ���̵��..


	MoveParticlesToTarget(m_pkMountedRidingPet); //������ ���� ��ƼŬ�� �꿡�Է� �̵�..
...
{ //CharRoot Node�� ���� �Ѵٸ� �ݵ�� �� �Լ����� ���� �Ѵ�. �� ž�� ���ο� ���� ���� ����� �޶��� �� �ֱ� �����̴�
...
bool PgActor::Update(float fAccumTime, float fFrameTime)
{
	...
	// �� InvisibleGroup�� Update���θ� �����Ѵ�. frameTime���� �����ϰ� ����ȴ�.
	...
	if (bUpdateThisFrame)
	{
		...
		TransitActorSpecular(fModifiedFrameTime);	//	���� �ٲٱ�
		TransitAlpha(fModifiedFrameTime);	//	������ �ٲٱ�
		TransitColor(fModifiedFrameTime);	//	������ �ٲٱ�
		UpdateBlink(fModifiedFrameTime);// ĳ���Ͱ� ���ڰŷ��� �Ѵٸ�
		//UpdateTwist();// ĳ���Ͱ� ���ۺ��� ���ƾ� �Ѵٸ�
	}
	...
	UpdateGodTime(fAccumTime);	//	���� Ÿ��
	UpdateDamageBlink(fAccumTime);//	����� ����ũ
	...
	//}
	m_bIsOptimizeSleep = true;	//������ �����ӿ��� ������ �ٽ� ����

	UpdateStatusEffect(fAccumTime, fFrameTime);


	//ž�� ���̸�, ���� ��Ʈ�ѷ��� PC�� ��Ʈ�ѷ��� ��ġ��Ų�� (�̵�, ����)
...
void	PgActor::DoLoadingFinishWork()
{
	...
	if (!m_iEquipCount && !m_bLoadingComplete)
	{
		...
		if (BM::GUID::NullData() != g_kParty.PartyGuid()
			&& pkPilot
			&& pkPC
			&& g_kParty.PartyGuid() == pkPC->PartyGuid()) // �� ��Ƽ���̸� ��Ƽ ��Ŭ ���δ�.
		{
		...
		if (pkPilot)
		{
			g_kMapMoveCompleteEventMgr.Pop(pkPilot->GetGuid());	// �� �̵����� ���� ���
		}
		...
		SyncMountPet(); //�ٸ� Player�� �ε��Ǹ� ž���� ����ȭ ���Ѿ� �Ѵ�.
		SyncActionOnAddUnit(pkPC); //�̵� ����ȭ (AddUnit() -> �ε��Ϸ� ��)
...
void	PgActor::NfyActorManagerChanged(bool bBefore)
{
	...
	else
	{
		...
		NiTimeController::StartAnimations(GetNIFRoot(), 0.0f); //PgWorld�� Attach�� �� �˾Ƽ� �ȴ�.
...
void	PgActor::UpdateHeadSize(float fFrameTime, float fAccumTime)	//	�Ӹ�ũ�� ������Ʈ
{
	...
	for (int i = 0; i < iTotalShapes; ++i)
	{
		...
		if (pkHitShape && pkHitShape->userData)
		{
			...
			if (pkPilot->GetAbil(AT_USE_PART_CHECK_COLLISION))
			{
				...
				if (shapename)
				{
					int	const iABVShapeIndex = pkActor->GetABVShapeIndex(shapename) + 1; //ABVShapeIdx�� 0�� ���ü��� �־� Abil�� +1�Ͽ� ���
...
int PgActor::SkinningParts(PgItemEx* pkParts, NiNode* pkNode, bool bAttach)
{
	...
	for (int iArrayIndex = 0; iArrayIndex < iArrayCount; ++iArrayIndex)
	{
		...
		if (pkChild->GetExtraData("HAS_NO_VALID_GEOMETRY")) // QUESTION? �ҹ��� �񱳴� �ǳ�.
		{
			continue;
		}
...
bool PgActor::AttachNoSkinningParts(PgItemEx* pkParts, char const* pcTargetDummy)
{
	...
	//	pcPartsNifPath �� Scene Root �� pcTargetDummy �� Attach ��Ų��.
	...
	// TODO : Weapon No�� Customizing�� �ʿ��ϴ�.
	if (pkParts->IsWeapon())
	{
...
bool PgActor::DetachParts(PgItemEx* pkParts)
{
	...
	// ������ ���⿴�ٸ� WeaponType�� 1000(Hand)���� �Ѵ�.
	if (pkParts->IsWeapon())
	{
		//	EndWeaponTrail();	//	Thread Safe ������ �ϴ� �ּ�ó��.
...
#endif
	{
		PgActorUtil::SetNameCulled(m_spNameText, m_spGuildNameText, m_spGuildMark, m_spGuardianMark, m_spTitleName, m_spAchievementTitle, m_spGIFTitle, m_spMyhomeMark, m_spCustomCountText, m_spDuelTitle, m_spEffectCountDown, true); // �ϴ� �����
...
bool PgActor::IsDrawable(PgRenderer* pkRenderer, NiCamera* pkCamera)
{
	...
	if (IsHide() || IsBlinkHide()) // ���ų� ����ũ ���� / !(�� & ������ ���)
	{
		if (!bThisIsEnemyActor)
		{
			PgCircleShadow::AddOnlyShadowToVisibleArrayRecursive(this, pkRenderer, pkCamera);	//	�׸��ڸ� �׸���
		}
		return false;
	}
...
void PgActor::Draw(PgRenderer* pkRenderer, NiCamera* pkCamera, float fFrameTime)
{
	...
	pkRenderer->CullingProcess_Deprecated(pkCamera, this, pkRenderer->GetVisibleArray_Deprecated(), true); // �׸� ��ü �ø�
	...
	CullingProcessParticle(pkCamera, pkRenderer->GetVisibleArray_Deprecated(), pkRenderer); // �׸� ��ƼŬ �ø�(Z-Test�����ϴ� ��ƼŬ)

	NewWare::Renderer::DrawActor(pkRenderer, this); // ������ �ø��� ��ü�� ��ƼŬ�� �׸�
...
bool PgActor::UpdateName(std::wstring const& rkName)
{
	...
	if (pPilot)
	{
		...
		CXUI_Font* pFont = g_kFontMgr.GetFont(FONT_NAME); // �̸�
			...
			&& pkUnit->IsUnitType(UT_PLAYER))
		{
			...
			if (pkPC)
			{
				if (BM::GUID::IsNotNull(pkPC->GuildGuid()))//��忡 ���ԵǾ� ������
				{
					bHaveGuild = true;
		...
		else
		{
			...
			if (m_spAchievementTitle)
			{
				...
				{
					pFont = g_kFontMgr.GetFont(FONT_TITLENAME); // ȣĪ
					if (pFont)
					{
...
bool PgActor::IsEnemy(PgActor* pkTarget)	//	���� ���ΰ�?
{
	if (!pkTarget)
	{
		return	false;
	...
	case UT_SUMMONED:
	{
		if (pkTargetUnit->IsUnitType(UT_MONSTER) || pkTargetUnit->IsUnitType(UT_OBJECT))	//	�÷��̾��� ���Ͱ� ���̴�
		{
			bIsEnemy = true;
		}
		else if (pkTargetUnit->IsUnitType(UT_PLAYER)
			|| pkTargetUnit->IsUnitType(UT_SUB_PLAYER)	// SUB_PLAYER�� Ÿ�� �ϰ� ������
	...
	case UT_BOSSMONSTER:
	{
		if (pkTargetUnit->IsUnitType(UT_PLAYER)		//	���Ͷ�� �÷��̾ ���̴�
			|| pkTargetUnit->IsUnitType(UT_SUMMONED))
		{
			bIsEnemy = true;
		}
...
bool PgActor::SetTargetAnimation(std::string const& rkAnimationName, bool const bActivate, bool const bNoRandom)
{
	...
	else
	{
		...
		if (pkController && pkController->GetCycleType() != NiTimeController::LOOP)
		{
			...
			pkAM->RebuildTimeline();	//	leesg213 2006.12.11 ��� ����� textkey event �� ���������� �߻��ȴ�.
...
bool	PgActor::StartSyncMove(PgAction* pkSyncMoveNextAction)
{
	...
	m_fSyncMoveStartTime = g_pkWorld->GetAccumTime();	//	���� �ð�
	m_kSyncMoveStartPos = GetPosition();
	//m_bNoFindPathNormal = true;	// �ӽ� SyncMove
...
bool	PgActor::UpdateSyncMove(float fSpeed, float fFrameTime)
{
	...
	if (fNextDistance >= fTotalDistance)
	{
		kNextPos = kTargetPos;
		bContinue = false;
		//m_bNoFindPathNormal = false;	// �ӽ� SyncMove
	}
...
bool PgActor::BeginSync(PgAction* pkAction, DWORD dwOvertime)
{
	...
	// Action Start Pos�� (0, 0, 0)�̸� Sync�� �� �ʿ䰡 ���ٴ� �Ŵ�.
...
bool PgActor::UpdateSync(float fFrameTime)
{
	...
	NiPoint3 kStartPos = m_kSyncPositionStart;
	NiPoint3 kSyncTargetPos = m_kSyncPositionTarget;
	if (eUType == UT_PET && IsRidingPet()) //���̵��� ž�½ÿ� ���� ��ġ ����ȭ�� ����������
	{
...
bool PgActor::ProcessAction(PgAction* pkAction, bool bInvalidateDirection, bool bForceToTransit)
{
	...
	if (pkAction && pkAction->GetEnable() && IsNowFollowing())
	{
		...
		{
			if (pkAction->GetActionOptionEnable(PgAction::AO_CAN_CHANGE_ACTOR_POS) || pkAction->GetActionType() == "EFFECT")	//	��ġ �̵��ϴ� �׼� �Ǵ� Effect���, ���󰡱⸦ �����
			{
	...
	if (!bActionReturn)
	{
		if ((bIsMyActor && IsNowFollowing()) || IsMyPet())
		{
			...
			{
				if (pkAction->GetActionOptionEnable(PgAction::AO_CAN_CHANGE_ACTOR_POS))	//	��ġ �̵��ϴ� �׼� ���н� ���󰡱⸦ �����
				{
					RequestFollowActor(GetFollowingTargetGUID(), EFollow_Cancel);
				}
			}
...
void	PgActor::OnTargetListModified(PgAction* pkNextAction)
{
	...
	{
		pkNextAction->OnTargetListModified(this, false);	//	�� Ÿ�ٸ���Ʈ�� ���� ��ũ��Ʈ ó��
		return;
	}

	//	�̰��� TargetList �� �ٲ������ �ǹ��Ѵ�.
	m_pkAction->OnTargetListModified(this, true);	//	���� Ÿ�ٸ���Ʈ�� ���� ��ũ��Ʈ ó��
	...
	m_pkAction->OnTargetListModified(this, false);	//	�� Ÿ�ٸ���Ʈ�� ���� ��ũ��Ʈ ó��
...
bool	PgActor::ProcessToggleAction(PgAction* pkNextAction)
{
	...
	if (GetActionToggleState(pkNextAction->GetActionNo()))
	{
		if (!IsStun())
		{
			...
			StartSkillCoolTime(pkNextAction->GetActionNo());	//	��Ÿ�� ����
			return false;
		}
	}
...
bool	PgActor::CheckRequirementForAction(PgAction* pkNextAction, bool const bShowFailMsg)
{
	...
	if (bIsSingleMode)
	{
		return	true;	//	�̱۸�忡���� ������ true
	}
#endif

	if (pkNextAction->GetActionType() != "IDLE")	//���̵��� �ƴ� ��쿡��
	{
		//	���� ���¿����� �ƹ� �ൿ�� �� �� ����.
	...
	{
		if (bIsMySubPlayer && IsHide()) //�ֵ��� ���Ͱ� ������ ���¶�� �׼��� �ߵ��Ǿ�� �ȵȴ� (ex: ���� �꿡 ž�� ���� ��)
		{
			return false;
		}
	...
	if (IsOnlyMoveAction())
	{
		...
		if (pkNextAction->GetActionType() != "IDLE"
			...
			&& pkNextAction->GetActionOptionEnable(PgAction::AO_KIND_OF_TRIGGER_ACTION) == false	// ��Ʈ���ſ� �׼��� �ƴ϶��
			)
		{
...
bool	PgActor::ProcessLeaveCurrentAction(PgAction* pkNextAction)
{
	...
	{
		if (m_pkAction->GetActionParam() == ESS_CASTTIME)		//	���� ĳ���� ���̾��ٸ�, ������ ���� �� �ִ�.
		{
			bSuccessfulLeave = true;
		}
	}
...
{
	//	ĳ���� Ÿ���� �ִ� �׼��� ���
	//	ActionParam �� PgAction::AP_CASTING �� �����Ѵ�.
	//	��Ÿ�� ��� PgAction::AP_FIRE �� �����Ѵ�.
...
void	PgActor::SetIgonreDamageEffect(PgAction* pkNextAction)
{
	...
	{
		bAddIgnoreDamageEffect = pkNextAction->GetActionOptionEnable(PgAction::AO_IGNORE_DMG_EFFECT) || bAddIgnoreDamageEffect; // ����� ����Ʈ�� �����Ұ��δ�.
	}
...
bool PgActor::DoAction(PgAction* pkNextAction, bool bForceToTransit)
{
	...
	if ("EFFECT" != pkNextAction->GetActionType() && false == m_pkActionEffectStack->IsEmpty() && false == bForceToTransit)
	{
		if (bIsMyActor)
		{
			return	false;	//	�׼�����Ʈ�� �������̶��, �ٸ� �׼����� ���̽�Ű�� �ʴ´�.
		}

		if (kUnitType != UT_PLAYER)
		{
			// TODO : ���� Broadcast�ؾ� �ϴ��� üũ.
			return	m_pkActionEffectStack->SaveLastAction(pkNextAction);	//	�÷��̾ �ƴ϶��, (���� or NPC) ������ �׼����� �����صΰ�, ����Ʈ�� ��� ����Ǿ�����, �׼��� �ڵ����� �������ش�.
		}
...
PgAction* PgActor::CreateActionForTransitAction(char const* pcNextActionName, bool bEnable, NiPoint3* pkActionStartPos, BYTE byDirection, int iActionNo)
{
	...
	pkAction->SetActionParam(ESS_FIRE);	//	����Ʈ ��
		...
		|| bIsMySubPlayer)
	{
		if (pkAction->GetSkillType() == EST_ACTIVE || pkAction->GetSkillType() == EST_TOGGLE)
		{
			...
			else // Skill Tree���� ã�� ���� ��� Cast �� Fire�� �������� ��ų�� ��찡 �ִ�.
			{
				...
				// _Fire �� ��ų�� ����� _Cast ��ų�� ID�� �� _Fire�� ���� ������ �ش��ϴ� ID�� �����Ѵ�.
	...
	else if (IsUnderMyControl())
	{
		...
		else if ((bIsEntity || bIsPet)	// �� ��ų�̰ų�, ��ȯü ��� �ϴ� ��ų�� ����� �ʾƵ� �Ǵ� ��ų�� ���
			&& iActionNo
			)
		{
			pkAction->SetActionNo(iActionNo);
...
bool	PgActor::AddDropItem(PgDropBox* pkItemBox)	//	�߰��� �� ������ true �ƴϸ� false
{
...
void	PgActor::DoDropItems(int iActionInstanceID, int iReqCount, float fJumpHeight)	//	iActionInstanceID �� �ش��ϴ� �����۵��� ������. -1 �� ��� ��� �������� ������. 
{
	if (!g_pkWorld)
	{
		return;
...
bool PgActor::CopyEquipItemFromMainPlayer(PgActor* pkActor, EInvType kInvType, EEquipPos kItemPos)
{
	...
	if (EQUIP_POS_HAIR_COLOR == kItemPos)	// �Ӹ����� PgItemEx�� ���� �ʿ䰡 ����. Hair�������� ���鶧 ������ ��.
	{
		return true;
	}
	...
	if (pkPilot)
	{
		...
		if (pkPlayer)
		{
			...
			if (0 < iItemNo)
			{// GetAdjustedItemNo()���� ���ϰ��� -1�ΰ�쵵 ������ �ϸ� �ȵǱ� ������ 0< 
...
{
	unsigned long const ulComboConnectionDelay = static_cast<unsigned long>(fMaxTime * 1000);	//	�޺� �Է� ���� ���� �ð�
...
{
	if (!m_bVisible || !m_bIsVisibleInFrustum) // m_bHide <- QUESTION: üũ�ؾ� �ϳ� ���ƾ� �ϳ�. Swift�� ������ m_bHide�� �ѳ��´�.
		return;
	...
	DrawParticle(pkRenderer, false);	//	Z Test ���� �ʴ� ��ƼŬ���� �������Ѵ�.
}
	...
	if (pkPilot)
	{
		...
		if (pkUnit)
		{
			...
			case UT_SUMMONED:
			{
				...
				NiAVObjectPtr	spTargetPoint = GetNodePointStar();
				if (spTargetPoint) // p_ef_star ��尡 ������ �ƿ� HP�ٸ� �������� �ʴ´�.
				{
...
bool PgActor::BeforeCleanUp()
{
	...
	//�꿡 ž�� ���̸� ������ �Ҹ��ؾ� �Ѵ�. (char_root ���� ��尡 �꿡 �پ������Ƿ� ���� ��ġ�� ����)
	if (IsRidingPet()) //����,�� �� �Ҹ� ������ �˼� �����Ƿ� ���� �Ҹ�Ǵ� ���� Unmount ���ش�.
	{
		UnmountPet();
	}
	...
	DetachNameNodes(GetNIFRoot()); // �̸����� ���ش�.
	m_spNameText = 0;
	m_spTitleName = 0;
	m_spGuildNameText = 0;
	m_spGuildMark = 0;
	...
	{
		g_pkLocalManager->ClearInputDirReverse();	//�� ĳ���͸� ��ǲ ���� �ݵǷ� �Ȱ� ������ �ʱ�ȭ
	}
...
void PgActor::ConcilDirection(NiPoint3& rkLookingDir, bool const bTurnRightAway)
{
	...
	// LookingDir �� Zero�̸� �ɹ������� ���� �־������ν� �Ŀ� NPC�� �н��� ã�� �� �ʿ��ϴ�.
	if (m_bNoConcil || NiPoint3::ZERO == rkLookingDir)
	{
		// NoConcil�̰ų�, LookingDir�� Zero�̸� �ƹ� �͵� ���� �ʴ´�.
		return;
	}
	...
	rkLookingDir.z = 0;
	rkLookingDir.Unitize();	//	leesg213 �߰���
	...
	if (kCross.SqrLength() < 0.0001f)
	{
		// LookingDir�� Y��� �� ������, UnitCross�� (0,0,0)�� �����Ѵ�.
		// �� ��� MovingDir�� y ���� ������ ������ �����ش�.
...
void PgActor::SetRotation(const NiQuaternion& kQuat, bool bForce)
{
	...
	SetRotate(kQuat);
	m_bIsOptimizeSleep = false;	//ȸ�������ϱ� ������Ű�� �ȵ�
}
...
Direction	PgActor::GetDirectionInverse(Direction kDirection)	//	kDirection �� �������� ���Ѵ�.
{
...
void PgActor::RestoreTransformation(char const* pcFirstAction)
{
	...
	DetachNameNodes(pkRoot); // �̸���(����, ��帶ũ, ��)�� �������� ���� Node���� ���ش�.
	...
	AttachNameNodes(pkAM->GetNIFRoot()); // �̸��� �ٽ� ���δ�.
...
void PgActor::Transformation(char const* pcNewModel, char const* pcFirstAction, int const iTransformEffectID)
{
	...
	DetachNameNodes(pkAM->GetNIFRoot()); // �������� �̸����� ���ְ�
	...
	if (m_bTransformed)
	{
		...
		AttachNameNodes(pkAM->GetNIFRoot()); // �̸���(����, ��帶ũ, ��)�� ������ Node�� �ٽ� �ٿ� �ش�.
...
//			}
//
//			g_kQuestMan.RemoveMiniQuestList(iQuestID);	// Mini ����â���� ����
//			g_kQuestMan.DelRecentBeginQuest(iQuestID);	// �ֱٿ� ������ ����Ʈ �������� ����
...
//				
//				g_kQuestMan.AddMiniQuestList(iQuestID);		// Mini ����â�� �߰�
//				g_kQuestMan.AddRecentBeginQuest(iQuestID);	// �ֱٿ� ������ ����Ʈ ������ �߰�
...
int	PgActor::GetPosChangeActionCount(const ActionQueue& kQueue)	//	ť�� ����ִ� �׼� �߿�, ĳ������ ��ġ�� �̵���Ű�� �׼��� ��� �ִ°�?
{
	int	iCount = 0;
	...
	for (ActionQueue::const_iterator itor = kQueue.begin(); itor != kQueue.end(); ++itor)
	{
		...
		{
			if (bCurrentActionCanChangeActorPos)	//	���� �׼��� �̵��׼��̶��, ���� ��ȯ ���� �̵����� �׼��̶�� �����Ѵ�.
			{
				++iCount;
			}
			continue;
...
bool	PgActor::ProcessFollowingActor()
{
	...
	if (!pkTargetActor)
	{
		if (m_kFollowInfo.IsTargetLost())
		{
			...
			if (30 < fElapsedTime)	//	30�� �̻� Ÿ���� �Ҿ��ٸ�, ����ٴϱ⸦ �����Ѵ�.
			{
				RequestFollowActor(GetFollowingTargetGUID(), EFollow_Cancel);
				return	true;
			}
...
void	PgActor::RequestFollowActor(BM::GUID const& kTargetActorGUID, EPlayer_Follow_Mode const kMode, bool const bForce)	//	kTargetActorGUID�� ���󰥰��� ������ ��û�Ѵ�
{
	if (!bForce)
	{
...
void	PgActor::ResponseFollowActor(BM::GUID const& kTargetActorGUID, EPlayer_Follow_Mode const kMode)	//	kTargetActorGUID�� ���󰥰��� ������ ��û�Ѵ�.
{
...
bool	PgActor::CheckCanFollow(const	BM::GUID& kTargetActorGUID, bool const bMsg)	//	kTargetActorGUID �� ���� �� �ִ��� üũ�ϰ� �޼����� �����ش�.
{
	...
	if (!bHasCorrectRelationship)
	{
		if (pkMyPlayer->PartyGuid() != BM::GUID::NullData())
		{
			...
			{
				bHasCorrectRelationship = true;	//	���� ��Ƽ��.
			}
		}
	}
	if (!bHasCorrectRelationship)
	...
	if (!bHasCorrectRelationship)
	{
		if (pkMyPlayer->CoupleGuid() != BM::GUID::NullData())
		{
			...
			{
				bHasCorrectRelationship = true;	//	Ŀ��
			}
		}
	}
	if (!bHasCorrectRelationship)
...
bool PgActor::FollowActor(const	BM::GUID& kTargetActorGUID)	//	kTargetActorGUID �� ���󰡵��� �Ѵ�.
{


	if (GetPilotGuid() == kTargetActorGUID)	//	���� ���� �i�ư�??? No way~
	{
		return	false;
	}
...
void	PgActor::AddFollowingMeActor(const	BM::GUID& kActorGUID)	//	���� ������� ���͸� �߰��Ѵ�. FollowActor() ���� �ڵ����� ȣ��ȴ�.
{
...
void	PgActor::RemoveFollowingMeActor(const	BM::GUID& kActorGUID)	//	���� ������� ���͸� �����Ѵ�. StopFollowActor() ���� �ڵ����� ȣ��ȴ�.
{
...
void PgActor::SeeLadder()
{
	...
	if (!bLadderFound)	//	�ڱ� ���ʿ� ������, ī�޶� �����ʿ� �ִ��� �ٽ� �ѹ� üũ
	{
...
// Return���� ����̸�, �ڽ��� �������� Target Object�� Axis�� +���⿡ �ִٴ� ���̴�. �����̸� �� �ݴ�
...
bool PgActor::WillBeFall(bool bDown, float fDistance)
{
	...
	// bDown == true �� ���� pkHitShape(bRet)�� false�̸� ��ȿ, true�̸� false �����ؾ� ��.
	// bDown == false �� ���� pkHitShape(bRet)�� true�̸� ��ȿ, false�̸� false �����ؾ� ��.
	...
	if(bFirstTry)
	{
		...
		// bDown == true�� ���� �� ��° Ray�� true�̸� ��ȿ, false�̸� false �����ؾ� ��.
		// bDown == false�� ���� �� ��° Ray�� false�̸� ��ȿ, true�̸� false�� �����ؾ� ��.
...
//	if( p3LeftOrRight.z > 0 )	{ return true; }	//�����϶� true
//
//	return false;
//}
...
{
	// �ƽ����� UserProp�� NiPhysXShapeGroup = <Group No> �� ���� ������ ��
	// (�׷��� ���� User Properties�� physX �޽��� ���� ���� ��� �ͽ���Ʈ �ش޶�� ��û�ϸ� �ȴ�)
...
void PgActor::GetNameEmoticon(std::wstring& rkOut)
{
	...
	if (pkUnit->IsInUnitType(UT_MONSTER) && g_pkWorld)
	{
		...
		if (bIsDependMonster
			&& !pkUnit->IsUnitType(UT_OBJECT)) // �ı� ������Ʈ�� ���⼭ ����
		{
			kTemp += L"E";
		}
	}
	...
	if (pkUnit->IsUnitType(UT_PLAYER))
	{
		...
		if (pkPlayer)
		{
			...
			if (bHaveCouple)
			{
				if (g_kPilotMan.IsMyPlayer(pkPlayer->CoupleGuid())) // �� Ŀ���� ��ó��
				{
					PgActor* pkMyActor = g_kPilotMan.GetPlayerActor();
					if (pkMyActor)
					{
						pkMyActor->UpdateName(); // �� ��Ʈ�� �αٹݱ�
					}
				}
	...
	if (GetPilot())
	{
		...
		case TEAM_NONE:
		{
			if (pkUnit->IsInUnitType(UT_MONSTER))
			{
				...
				int const iGrade = pkUnit->GetAbil(AT_GRADE);
				if (EMGRADE_BOSS == iGrade || EMGRADE_ELITE == iGrade)	//������ ���� ���ص� �� ����� �켱�Ѵ�
				{
					/*switch(iGrade)
					{
			...
			else if (pkUnit->IsUnitType(UT_PLAYER))
			{
				...
				if (pkPC)
				{
					...
					if (!bNullPartyGuid
						&& g_kParty.PartyGuid() == rkPartyGuid)//���� ��Ƽ����
					{
						rkOut = std::wstring(_T("{C=0xFF00D1FF/}"));
					}
					//else if( !bNullPartyGuid )//�ٸ� ��Ƽ��
					//{
...
void	PgActor::PickUpNearItem(bool bCheckCaller, float const fPickRange, int const iPickCount)	//	��ó�� �������� ���´�.
{
	if (!g_pkWorld)
	{
		return;
	...
	for (int i = 0; i < iCount; ++i)
	{
		...
		if (pkDropBox->IsMine() == false)	//	������ �ƴϸ� �н�
		{
...
bool PgActor::SetPosition(NiPoint3 const& rkTranslate)
{
	...
	/// Comment : PhysX�����ڰ� ���� ������Ʈ �ÿ� Gamebryo Object�� ��ǥ�� ���� �ֱ� ������ 
	///	�� �����ӿ� GetTranslate()�� �ϸ� ��ǥ�� �� ������ ��߳���. ������ AVObject::SetTranslate()�� ���� ���ش�. 
	...
	/// Comment ; NxCapsuleController::setPosition()�� bool�� �����ϴµ�, �޴���(2.6.2)�� ���� ����� �׻� true�� �����Ѵٰ� �Ǿ� �ִ�.
	return true;
}
...
float	PgActor::GetJumpTime()	const
{
	return	m_fJumpTime;
}
	...
	if (GetWorld())
	{
		...
		if (pCamera)
		{
			...
			if (pCamera->WorldPtToScreenPt3(pos, m_kNormalizedActorPosByCamera.x, m_kNormalizedActorPosByCamera.y, m_kNormalizedActorPosByCamera.z, 0.001f))
			{
				...
				if (m_kNormalizedActorPosByCamera.z >= 0.90f) // z�� Ŭ���� �ָ� �ִ� ���̴�.
				{
...
bool PgActionSay::GetRandomSay(ContSayItem const& rkCont, SSayItem const* pkPrev, const SSayItem*& pkOut)const
{
	...
	if (iMin != iMax)//������ 1��
	{
		...
		{
			if (rkCont.at(iMin) == *pkPrev)//������ Min�� ������ Ȯ��
			{
				iCur = iMax;
			}
			else
	...
	if (m_kIdleSayList.size() > 0)
	{
		...
		case SAS_None:
		{
			...
			if (bFindSay)
			{
				...
				eRet = m_kCurSayStatus = SAS_Run; //���⼭ ��ǳ�� ����
			}
			else
			{
				//������ ��쿣 50�� ��ŭ ħ��
		...
		case SAS_Run:
		{
			eRet = m_kCurSayStatus = SAS_Wait; //����
		}break;
		case SAS_Wait:
		{
			if (UpTime() < 0.f)
			{
				if (m_pkCurSayItem) UpTime(m_pkCurSayItem->fDelay);//Delay �ð� ����
				eRet = m_kCurSayStatus = SAS_Delay; //���� �� ���
			}
		}break;
		case SAS_Delay:
...
bool CheckHPMPForAction(CSkillDef const* pkSkillDef, CUnit const* pkUnit, bool const bShowFailMsg, bool const bShowNILog)
{
	...
	// ���� ������
	int iNeedAwake = pkSkillDef->GetAbil(AT_NEED_AWAKE); // ������ ������ ���밪 ����
	int const iNeedAwakePer = pkSkillDef->GetAbil(AT_NEED_MAX_R_AWAKE); // ������ �������� ��ü�� %�� �Ҹ�
	if (0 < iNeedAwakePer)
	{
...
bool PgActor::CheckOutLobby(PgTrigger* pTrigger)
{
	...
	if (false == GetLeaveExpedition())	// ���� ���°� false�� ��
	{
		if (true == PgClientExpeditionUtil::IsInExpedition())	// �����뿡 ���� ������
		{
			if (g_pkWorld->IsHaveAttr(GATTR_EXPEDITION_LOBBY))	// ������ �κ� �´��� ���Ѵ�.
			{
				SetCurrentTrigger(pTrigger);	// ���� Ʈ���Ÿ� �����ϰ� �˾�â�� ����.
...
{// m_kControllerShapeHit �߿��� ����� �ʱ�ȭ�� �ܺη� ���� ���� �ʾƼ� ���� �Լ��� ����
