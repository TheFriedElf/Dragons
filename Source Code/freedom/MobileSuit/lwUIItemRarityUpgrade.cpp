...
int const ISURANCE_ITEM_NO_BASE = 98000040;	//보험 아이템.
...
void Recv_PT_C_M_ANS_ITEM_RARITY_UPGRADE(BM::Stream* pkPacket)
{
	...
	if(IRUR_NONE!=kStruct.Result())
	{
		...
		case IRUR_SUCCESS://	= 1,
			{
				...
				{
					wstrWarnMessage = ::TTW(1480);;//	저주를 풀었습니다.
				}
				else
				{
					wstrWarnMessage = ::TTW(1401);;//	제련에 성공 하였습니다
				}
			}break;
		...
		case IRUR_INVALID_PROPERTY:	// 정의 되지 않은 속성 이다.
			{
		...
		case IRUR_FAIL:				// 실패
			{
				if (E_PPTY_CURSE==g_kItemRarityUpgradeMgr.AttachElement())
				{
					wstrWarnMessage = ::TTW(1481);;//	저주풀기 실패
				}
				else
				{
		...
		case IRUR_FAIL_AND_BROKEN:	// 실패해서 깨짐
			{
				wstrWarnMessage = ::TTW(1405);;
			}break;
		case IRUR_NOT_ENOUGH_MONEY: //	
			{
				wstrWarnMessage = ::TTW(1407);;//돈이 모자랍니다.
			}break;
		case IRUR_NOT_FOUND_TARGET_ITEM: //	= 2,//재료 부족
		...
		g_kItemRarityUpgradeMgr.StartTime(g_pkApp->GetAccumTime());	//결과 이펙트용
...
void lwUIItemRarityUpgrade::OnTick(lwUIWnd kWnd)
{
	...
	{
		g_kItemRarityUpgradeMgr.InProgress(false);	//시간 다 됬으니까
...
bool PgItemRarityUpgradeMgr::SetSrcItem(const SItemPos &rkItemPos)
{
	...
	int iErrorNo = 59002;//! 크래프트 할 수 없는 아이템.
	if (E_PPTY_CURSE==AttachElement())
	{
		iErrorNo = 59005;
	}
...
void PgItemRarityUpgradeMgr::DisplayNeedItemIcon(int const iNeedIndex, XUI::CXUI_Wnd *pWnd)
{
	...
	case RIT_SOUL:
		{
			if (IsOnlyUseSoul() && RIT_SOUL!=iNeedIndex)	//영력만 올리는거면
			{
				goto __HIDE;
			}
	...
	case RIT_PROBABILITY:	{ bGray = (SItemPos::NullData() == m_kProbabilityItemPos)?(true):(false);	}break;	//내 속성
	case RIT_INSUR_ITEM:	{ bGray = (SItemPos::NullData() == m_kInsureItemPos)?(true):(false);		}break;	//보험
	default:
		{
			goto __HIDE;
		}
...
bool PgItemRarityUpgradeMgr::SendReqRarityUpgrade(bool bIsTrueSend)
{
	...
	if(IsChangedGuid()){goto __FAILED;}//guid 바뀌었으면 찾아가든가.
...
void PgItemRarityUpgradeMgr::SetMaterialItem(XUI::CXUI_Wnd* pWnd, int iType, bool bNoBuyMsg)
{
	...
	if( iType == RIT_INSUR_ITEM )
	{
		...
		if( rkItemCont.size() > 1 )
		{
			...
			std::pair<UIItemUtil::CONT_CUSTOM_PARAM::iterator, bool> ParamRst = kParam.insert(std::make_pair(std::wstring(L"CALL_UI"), UIItemUtil::EICUT_RARITY_UPGRADE));
			if( !ParamRst.second )
			{
				return;
			}
...
BM::vstring PgItemRarityUpgradeMgr::GetExplane()	//설명
{
	return m_kExplane;
}
